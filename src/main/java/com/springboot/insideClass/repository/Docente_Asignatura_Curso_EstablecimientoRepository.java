...
public interface Docente_Asignatura_Curso_EstablecimientoRepository extends JpaRepository<Docente_Asignatura_Curso_EstablecimientoEntity, Long> {
    ...
    @Query(value = "select dace.* from t_docente_asignatura_curso_establecimiento dace\n" +
            ...
            "inner join t_asignatura a on a.asignatura_id = da.docente_asignatura_id\n" +
            "inner join t_curso_establecimiento ce on ce.curso_establecimiento_id = dace.dace_curso_establecimiento_id\n" +
            "inner join t_curso c on c.curso_id = ce.curso_establecimiento_curso_id\n" +
            "inner join t_establecimiento e on e.establecimiento_id = ce.curso_establecimiento_establecimiento_id\n" +
            "where \n" +
            ...
            "(c.curso_id = :curso_id or :curso_id = -1 ) and (c.curso_nivel = :curso_nivel or :curso_nivel = '-1') and (c.curso_nombre = '-1' or :curso_nombre = '-1') and \n" +
