...
INSERT INTO [dbo].[t_utp]
 ...
 IF EXISTS (SELECT * FROM sysobjects WHERE name = 'fn_InfoMatricula')  DROP FUNCTION fn_InfoMatricula;


               Create FUNCTION [dbo].[fn_InfoMatricula]  (@establecimiento bigint, @persona_run varchar(100), @curso_id bigint, @vigencia bigint, @apoderado_run varchar(20))
                RETURNS TABLE AS RETURN(
                select p.persona_run as alumno_rut,
                        p.persona_nombre as alumno_nombre,
                        p.persona_apellido_paterno as alumno_apellido_paterno,
                        p.persona_apellido_materno as alumno_materno,
                        apoderado.apoderado_persona_run as apoderado_run,
                        apoderado.persona_nombre as apoderado_nombre,
                        apoderado.persona_apellido_paterno as apoderado_apellido_paterno,
                ...
                inner join t_establ e on ce.curso_establ_establ_id = e.establ_id
                     where (@establecimiento = -1 or e.establ_id = @establecimiento)
...
IF EXISTS (
            ...
            AS
            RETURN (
                SELECT c.*, t1.curso_id, t1.curso_nombre, t1.alumno_rut, t1.alumno_nombre, t1.alumno_apellido_paterno, t1.alumno_materno, t1.establ_id, t1.establ_nombre
                FROM t_comunicaciones c
                INNER JOIN fn_InfoMatricula(@establ_id, @persona_run, @curso_id, 1, '-1') t1 ON c.curso_establ_id = t1.curso_establ_id
                WHERE (@establ_id = -1 OR t1.establ_id = @establ_id)
                    AND (@persona_run = '-1' OR t1.alumno_rut = @persona_run)
                    AND (@curso_id = -1 OR t1.curso_id = @curso_id)
                    AND (
                        (@fecha = '-1') OR
                        (
                            (
                                ISDATE(@fecha) = 1
                                AND TRY_CONVERT(DATE, @fecha, 103) BETWEEN DATEFROMPARTS(YEAR(c.fecha), 1, 1) AND DATEFROMPARTS(YEAR(c.fecha), 12, 31)
                            )
                            OR
                            (
                                ISDATE(@fecha) = 0
                      ...
                      IF EXISTS (
                                ...
                                CREATE  FUNCTION [dbo].[fn_InfoAsistencia] (
                                                                            @run varchar(100),
                                                                            @establ_id bigint,
                                                                            @fecha varchar(100)
                                                                        )
                                                                        RETURNS TABLE AS
                                                                        RETURN (
                                                                          	select * from t_asistencia a inner join  fn_InfoMatricula(-1, -1, -1, 1, '-1') t1 on t1.matricula_id = a.asistencia_matricula_id

                                                                            WHERE @fecha = '-1' or  a.fecha = @fecha
                                											and @run = '-1' or t1.alumno_rut =  @run
                                												and @establ_id = '-1' or t1.establ_id =  @establ_id
                                                                        );
          ...
          CREATE FUNCTION fn_InfoApoderado (
              ...
              @vigencia bigint,
              @apoderado_run varchar(20)
          )
          RETURNS TABLE
          AS
          RETURN (
              SELECT per.persona_run, per.persona_nombre, per.persona_apellido_paterno, per.persona_apellido_materno,
                     per.persona_numero_celular, per.persona_numero_telefonico, per.persona_fecha_nacimiento, per.persona_sexo,
                     u.email, t1.establ_id, t1.establ_nombre
              FROM t_apoderado apoderado
              INNER JOIN t_persona per ON apoderado.apoderado_persona_run = per.persona_run
              INNER JOIN fn_InfoMatricula(@establecimiento, -1, -1, @vigencia, @apoderado_run) t1 ON t1.apoderado_run = apoderado.apoderado_persona_run
              INNER JOIN t_usuario u ON per.persona_run = u.usuario_persona_run
              WHERE
                  (@establecimiento = -1 OR t1.establ_id = @establecimiento)
                  AND (@apoderado_run = '-1' OR apoderado.apoderado_persona_run = @apoderado_run)
          );